...
- **Dropped from the draft**: all inline explanatory prose and links (e.g. the Ingress-vs-Gateway-API blog link under the old Services section) — `Contents.md` keeps only the original 4 links under "Further Reading / Links". Such supplementary links can be reintroduced in if useful there.
